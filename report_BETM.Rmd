...
output:
  html_document:
    ...
    keep_md: true
params:
  Save_Path: models/BETM_spliced_ep2000_nlv32_bs1024_lr0.01_train_size0.9_pip0rho_0.1_klbeta_10.0v1
---
...
```{r read genes}
.genes <- fread(paste0(Save_Path, "/genes.csv.gz"), header = TRUE)
...
# notice that all BETM file are named with rho as it is "shared"
target <- "rho"
...
if(!dir.exists(paste0(Save_Path, "/figures/"))){
...
IMMUNESIG.lol <- IMMUNESIG.human.db %>% dplyr::select(gene_symbol, gs_name) %>% make.gs.lol()

HALLMARK.human.db <- msigdbr::msigdbr(species = "human",
										category = "H")

HALLMARK.lol <- HALLMARK.human.db %>% dplyr::select(gene_symbol, gs_name) %>% make.gs.lol()
...
```{r fgsea.alltopic.immunesig_rho, fig.width=9, fig.height=5, echo = FALSE, eval = FALSE}
...
```{r fgsea.alltopic.hallmark_rho, fig.width=9, fig.height=5, echo = FALSE, eval = FALSE}
...
```{r fgsea.alltopics.kegg_rho, fig.width=9, fig.height=5, echo = FALSE, eval = FALSE}
...
```{r aggregate.gsea_rho, echo=FALSE, eval = FALSE}
...
bias_gene <- 
            melt(bias_gene_rho, variable.name = "gene",value.name = "bias", id.vars = "param")
