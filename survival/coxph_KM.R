...
model.path <- "models/BDeltaTopic_allgenes_ep2000_nlv32_bs1024_combinebyadd_lr0.01_train_size1.0_pip0rho_0.1_pip0delta_0.1_klbeta_10.0v1"
...
#Heatmap(weight.mat[,topics_to_show])
...
topics_to_show <- paste0("topic", c("4","11","7","29","26","6","10"))

mat_to_plt_heatmap_CA <- mat_to_plt_heatmap[,topics_to_show]
mat_to_plt_heatmap_AU <- mat_to_plt_heatmap[,topics_to_show]
mat_to_plt_heatmap_US <- mat_to_plt_heatmap[,topics_to_show]

mat_to_plt_heatmap <- rbind(mat_to_plt_heatmap_CA, mat_to_plt_heatmap_AU, mat_to_plt_heatmap_US)

ha = c(rep("CA", nrow(mat_to_plt_heatmap_CA)),
rep("AU", nrow(mat_to_plt_heatmap_AU)),
rep("US", nrow(mat_to_plt_heatmap_US))
pdf(paste0("survival/figures/heatmap/", "heatmap_", "combined","_", target,".pdf"))
Heatmap(t(mat_to_plt_heatmap[,topics_to_show]),
    name = "deltaTopics",
...
pdf(paste0("survival/figures/heatmap/", "heatmap_zscore_", location, "_", target,".pdf"))
Heatmap(scale(mat_to_plt_heatmap),
    name = "deltaTopics",
...
file = paste0("survival/", "coxph_res_", location, "_", target, ".txt")
if(!exists(file)){
...
for(topic in paste0("topic", 1:32)) {
    ...
    pdf(paste0("survival/figures/KM/", "KM_", topic,"_2group_",location,"_", target,".pdf"))
    ...
    plt_3group <- ggsurvplot(fit_3group, 
            ...
            title="Kaplan-Meier Curve for PDAC Survival", 
            risk.table.height=.25)
    pdf(paste0("survival/figures/KM/", "KM_", topic,"_3group_",location,"_", target,".pdf"))
